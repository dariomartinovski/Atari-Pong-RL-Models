...
if __name__ == "__main__":
    ...
    # Duelling DQN model is different because it uses advantage estimation in the forward function, 
    # DQN and DDQN are the same model, but they use the network and tgt_network differently
    ...
    # we use Adam optimizer because it gives great results
    optimizer = optim.Adam(net.parameters(), lr=params["learning_rate"])
    total_rewards = []
    frame_idx = 0
    ts_frame = 0
    ts = time.time()
    best_mean_reward = 17.5
    stop_saving = False
    ...
    while True:
        ...
        if reward is not None:
            ...
            # we save the current trained model after 100 games every 10 games, 
            # for assurance that we don't lose the progress if the game crashes
            num_of_games = len(total_rewards)
            if num_of_games >= 100 and num_of_games % 10 == 0 and not stop_saving:
                torch.save(net.state_dict(), f"{params['save_path']}/{args.env}-{num_of_games}.dat")
                print(f"Saved on {num_of_games} game")

            # if there's a bigger reward than the inital 17.5 then the model is saved, 
            # and it stops saving the model every 10 games. And after that it checks if 
            # the model's results are better than the ones before
            # if it reaches 19.5 reward the model stops training
